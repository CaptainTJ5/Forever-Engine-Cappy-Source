	...
	public function new(curStage)
	{
		...
		switch (curStage)
		{
			...
			case 'valley':
				...
				curStage = 'valley';
				var bg:FNFSprite = new FNFSprite(-280, -200).loadGraphic(Paths.image('backgrounds/' + curStage + '/NushValley1'));
				bg.antialiasing = (!Init.trueSettings.get('Disable Antialiasing'));
			...
			case 'valley2':
				PlayState.defaultCamZoom = 0.85;
				curStage = 'valley2';
				var bg:FNFSprite = new FNFSprite(-280, -200).loadGraphic(Paths.image('backgrounds/' + 'valley' + '/NushValley2'));
				bg.antialiasing = (!Init.trueSettings.get('Disable Antialiasing'));
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
			...
			case 'valley3':
				PlayState.defaultCamZoom = 0.85;
				curStage = 'valley3';
				var bg:FNFSprite = new FNFSprite(-280, -200).loadGraphic(Paths.image('backgrounds/' + 'valley' + '/NushValley3'));
				bg.antialiasing = (!Init.trueSettings.get('Disable Antialiasing'));
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
			...
			case 'arcade':
				...
				curStage = 'arcade';
				var bg:FNFSprite = new FNFSprite(-500, -250).loadGraphic(Paths.image('backgrounds/' + 'kapi' + '/sunset'));
				bg.antialiasing = (!Init.trueSettings.get('Disable Antialiasing'));
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
			...
			case 'valleyEX':
				...
				if (PlayState.SONG.song.toLowerCase() == 'potential')
					bg.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/NushValleyRock');

				bg.frames = Paths.getSparrowAtlas('backgrounds/' + curStage + '/NushValleyEX');
				bg.animation.addByPrefix('idle', 'background 2', 24);
				bg.animation.play('idle');
				bg.scrollFactor.set(0.8, 0.9);
				bg.scale.set(6, 6);
